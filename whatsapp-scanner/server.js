...
const HISTORICO_LIMITE = Number(process.env.HISTORICO_LIMITE || 200);
...
app.post('/api/sincronizar', async (req, res) => {
  ...
  sincronizarHistorico(clientRef, limite).catch((e) => console.error('[sync]', e.message));
  res.json({ ok: true, iniciado: true, limitePorConversa: limite });
});
...
function iniciarWhatsApp() {
  ...
  client.on('ready', async () => {
    ...
    if (!estado.sincronizado && process.env.SYNC_AO_CONECTAR !== '0') {
      sincronizarHistorico(client, HISTORICO_LIMITE).catch((e) => console.error('[sync]', e.message));
    }
  });
 ...
 */
async function sincronizarHistorico(client, limitePorConversa) {
  if (estado.sincronizando) return;
  estado.sincronizando = true;
  estado.progressoSync = { atual: 0, total: 0, encontrados: 0 };
  console.log(`[sync] iniciando varredura do histórico (até ${limitePorConversa} msgs/conversa)...`);
  try {
